...
def _sample_by_page_type(urls, per_type):
    ...
    실측(soup 포함) 분류와 99% 일치 검증됨. 'unknown'은 다른 국가 run 관례대로 제외.
    """
    ...
    for pt in sorted(buckets):
        out.extend(buckets[pt][:per_type])
    print(f"[render-audit] page_type 샘플(≤{per_type}/type): "
          + " ".join(f"{pt}={min(len(v),per_type)}" for pt, v in sorted(buckets.items())))
    return out
